...
var textMore = document.querySelectorAll('.about__text--inactive');
var textMoreMobile = document.querySelector('.about__text--inactive-half');

var onMoreBtnClick = function onMoreBtnClick(evt) {
  evt.preventDefault();
  aboutMoreBtn.classList.toggle('about__link--active');
  textMore.forEach(function (text) {
    return text.classList.toggle('about__text--inactive');
  });
...
var Forms = /*#__PURE__*/function () {
  ...
  _createClass(Forms, [{
    ...
    key: "_checkPhoneMask",
    value: function _checkPhoneMask(evt) {
      var el = evt.target;
    ...
    key: "_activatePhoneMask",
    value: function _activatePhoneMask() {
    ...
    key: "_activateLocalStorage",
    value: function _activateLocalStorage() {
      try {
        if (this._phoneField) {
          this._storagePhone = localStorage.getItem("".concat(this._phoneField.getAttribute('id')));
        }

        if (this._questionField) {
          this._storageQuestion = localStorage.getItem("".concat(this._questionField.getAttribute('id')));
        }

        if (this._nameField) {
          this._storageName = localStorage.getItem("".concat(this._nameField.getAttribute('id')));
        }
      } catch (err) {
    ...
    key: "_checkNameField",
    value: function _checkNameField(evt) {
      if (evt.target.value === '') {
        evt.target.classList.add('input--error');
      } else {
        evt.target.classList.remove('input--error');
    ...
    key: "_useLocalStorage",
    value: function _useLocalStorage() {
      if (this._isStorageSupport) {
        if (this._phoneField) {
          localStorage.setItem("".concat(this._phoneField.getAttribute('id')), this._phoneField.value);
        }

        if (this._nameField) {
          localStorage.setItem("".concat(this._nameField.getAttribute('id')), this._nameField.value);
        }

        if (this._questionField) {
          localStorage.setItem("".concat(this._questionField.getAttribute('id')), this._questionField.value);
        }
      }
    }
  }, {
    key: "_activateFormValid",
    value: function _activateFormValid() {
      if (!this._form) {
        return;
      }
      ...
      if (this._getScrollbarWidth()) {
        this._sections.forEach(function (section) {
          ...
          if (!sectionBackImageExist) {
            section.style.paddingRight = "".concat(parseInt(sectionPaddingRight, 10) + _this._getScrollbarWidth(), "px");
          } else {
            section.style.backgroundSize = "calc(100% - ".concat(_this._getScrollbarWidth(), "px) 100%");
            section.style.paddingRight = "".concat(parseInt(sectionPaddingRight, 10) + _this._getScrollbarWidth(), "px");
          }
        });
